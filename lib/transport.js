...
WebRTCTransport.prototype._onSignal = function(signallerMessage) {
  ...
  if(this._handshakesToContacts[signallerMessage.handshakeID]) {
    var peer = this._handshakesToContacts[signallerMessage.handshakeID].peerConnection;
  }
  if(!peer){
...
WebRTCTransport.prototype._send = function(data, contact) {
  ...
  var handshakeID = hat();
  var rcLogger = new Logger(4);
  if(!contact.peerConnection || contact.peerConnection.destroyed){
    rcLogger.info("DRC Creating new peer inside _send!");
    contact.peerConnection = this._createPeer(contact.nick, handshakeID, true);
    this._handshakesToContacts[handshakeID] = contact;
  }
  else {
    rcLogger.info("DRC Using old peer inside _send!");
  }
  contact.peerConnection.on('connect', function() {
    contact.peerConnection.send(data.toString());
    // setTimeout(function() {
    //   newPeer.destroy();
    // }, 1000);
  });
